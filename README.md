...
Русская документация по dual-camera DLCLive системе для Daheng/Galaxy камер и
Open Ephys. Основная задача системы: получить видеопоток с двух камер, посчитать
...
1. Этот файл, чтобы понять всю систему целиком.
...
4. `docs/README_DLC_live.md`, если нужен старый однокамерный режим.
...
| --- | --- | --- |
| Daheng/Galaxy камеры | Отдают уже аппаратно обрезанные кадры `1920x220`. | Не считают точки, углы или TTL. |
...
Ключевая мысль: в нормальном режиме Python отправляет не готовые `ttl_lines`, а
...
Ниже дотошно расписан путь одного live pair от камер до TTL.
...
Если мыши нет или модель не видит точки, triplet будет `False`, угол будет
`nan/None`, а TTL обычно будет `0x00`. Это нормальный результат пустого кадра,
а не признак поломки UDP или плагина.
...
Плагин не дергает физическую линию напрямую. Он создает Open Ephys event channel:
