...
дорожке.** Две камеры смотрят на животное, DLCLive считает позу, система
определяет, в какой точке шагового цикла находится нога, и выдаёт TTL ровно в
заданный процент фазы. Стимул подаётся не «в опору» и не «в перенос», а в
конкретный момент цикла.

Контура два, и смешивать их нельзя:

| | отвечает на вопрос | частота | что делает |
| --- | --- | --- | --- |
| **быстрая петля** | **когда** стимулировать | 52 Гц, каждый кадр | фаза шага → поджиг на целевом проценте → TTL. Детерминированная, без обучения. Это контур безопасности. |
| **медленная петля** | **чем** стимулировать | раз в пробу, ~10 шагов | подбор амплитуды, частоты, длительности серии и межканального сдвига байесовской оптимизацией |

## Что за чем идёт

```mermaid
flowchart LR
    subgraph PY["Python · dual_rt_dlc_live.py"]
        direction TB
        CAM["2 камеры Daheng<br/>1920×220, аппаратный кроп"]
        DLC["DLCLive / PyTorch<br/>ResNet-50, ~18 мс"]
        PHASE["процент фазы шага<br/>StreamingPhasePercent"]
        CAM --> DLC --> PHASE
    end

    subgraph OE["Open Ephys · DualDLCLiveBridge"]
        direction TB
        SOCK["приём пакета<br/>сокетный поток"]
        ANG["фильтр точек,<br/>triplet, угол"]
        WORD["сборка TTL word"]
        SOCK --> ANG --> WORD
    end

    STIM["стимулятор"]

    DLC -.->|"режим pose:<br/>сырые точки"| SOCK
    PHASE ==>|"режим ttl:<br/>готовые линии"| SOCK
    WORD --> TTL["TTL-событие<br/>Dual DLCLive TTL"]
    TTL --> STIM
```

**Два режима, и это важно.** Пунктир — режим `pose`, сегодняшний по умолчанию:
Python шлёт сырые координаты, а решение «какая линия должна быть активна»
принимает C++ плагин. Жирная стрелка — режим `ttl`: процент фазы считает Python
и присылает готовые линии, плагин их только передаёт. Фазовый триггер работает
**только** во втором режиме, см. `PHASE_TRIGGER_ENABLED` в
[`python/config_dual_rt_dlc_live.py`](python/config_dual_rt_dlc_live.py).

### Что происходит внутри плагина

```mermaid
flowchart TD
    IN["UDP-пакет DDLP/v1<br/>приходит в любой момент"] --> PARSE["разбор пакета<br/>сокетный поток"]
    PARSE --> VALID{"точки достоверны?<br/>полоса угла,<br/>скорость угла,<br/>смещение от hip-ankle"}
    VALID -->|"нет"| DROP["линия не поднимается"]
    VALID -->|"да"| TRIP["выбор triplet,<br/>расчёт угла задней ноги"]
    TRIP --> QUEUE["слово TTL в очередь"]
    WD["сторож<br/>тишина дольше таймаута"] -->|"гасит все линии"| QUEUE
    QUEUE --> PROC["вызов process<br/>ПОБЛОЧНО, не сразу"]
    PROC --> EVT["событие на канале<br/>Dual DLCLive TTL"]
```

Узкое место здесь — последний шаг. Пакет приходит на сокетном потоке в любой
момент, а `process()` вызывается поблочно, поэтому TTL встаёт в начало
**следующего** блока. Задержка равномерна от нуля до длительности блока: при
буфере 1024 отсчёта на 30 кГц это до 34 мс с джиттером 9.9 мс. **Размер буфера
Open Ephys — параметр системы, а не настройка записи.**

Сторож закрывает отказ, который иначе никак не виден: если Python замолчал —
упал, отвалилась камера, порвался сокет, — линии останутся в том состоянии, в
котором были. Поднятая линия так и останется поднятой. Сторож гасит их сам.

### Две петли целиком

![Быстрая и медленная петли](docs/media/report/fig9_two_loops.png)

Быстрая петля слева, медленная справа. Обе сходятся на стимуляторе, но дают ему
разное: быстрая — **триггер** (когда), медленная — **программу** (чем). Канал
для передачи программы пока не выбран: по TTL амплитуду не передашь, и это
открытый вопрос. Счёт походки для медленной петли берётся из тех же точек DLC,
что и фаза, — отдельного канала измерения не нужно.

## Куда идти

**Поставить и запустить с нуля** — [`docs/INSTALL_AND_RUN.md`](docs/INSTALL_AND_RUN.md):
клонирование, драйвер, venv, SDK, модель, плагин.

**Провести эксперимент** — [`docs/RUN_EXPERIMENT.md`](docs/RUN_EXPERIMENT.md):
запуск OE и плагина, угловой триггер, правила достоверности, запись, проверка
TTL, параметры сторожа.

**Собрать плагин после правок** — [`docs/BUILD_PLUGIN.md`](docs/BUILD_PLUGIN.md):
сборка Release, деплой на целевой OE, прогон тестов gtest и грабли с устаревшей
DLL в тестовой папке.

### Что уже измерено

| Документ | О чём |
| --- | --- |
| [`docs/REPORT_2026-08-19.md`](docs/REPORT_2026-08-19.md) | **Главный отчёт.** Фазозависимая стимуляция целиком: метрики, задержки по стадиям, найденные баги, медленная петля, бюджет сессии, что измерено и что нет. |
| [`docs/GAIT_PHASE_PERCENT_2026-08-15.md`](docs/GAIT_PHASE_PERCENT_2026-08-15.md) | Процент фазы шага: метод, поправки, геометрия против таймера, достижимые проценты. |
| [`docs/REMOTE_RESEARCH_2026-08-15.md`](docs/REMOTE_RESEARCH_2026-08-15.md) | Что имеет смысл сделать до эксперимента, с литературой. |

### Как устроены части

| Документ | О чём |
| --- | --- |
| [`python/README_OPEN_EPHYS_BRIDGE.md`](python/README_OPEN_EPHYS_BRIDGE.md) | Python runtime: камеры, DLCLive, binary fast-mode, профилировщик, логи. |
| [`open_ephys_plugin/DualDLCLiveBridge/README.md`](open_ephys_plugin/DualDLCLiveBridge/README.md) | C++ плагин: параметры, входные пакеты, фильтрация, TTL. |
| [`docs/README_DLC_live.md`](docs/README_DLC_live.md) | Однокамерный `rt_dlc_live.py` — изначальный MVP, нужен для отладки одной камеры и модели. |
| [`docs/CODE_WALKTHROUGH_2026-06-15.md`](docs/CODE_WALKTHROUGH_2026-06-15.md) | Разбор кода по файлам. |
| `camera_configs/*.txt` | Снимки конфигов Daheng/Galaxy для левой и правой камер. |

### Ключевые модули

| Файл | Что делает |
| --- | --- |
| [`python/dual_rt_dlc_live.py`](python/dual_rt_dlc_live.py) | Боевой цикл: камеры, спаривание, DLCLive, отправка по UDP. |
| [`python/realtime_phase_percent.py`](python/realtime_phase_percent.py) | `StreamingPhasePercent` — процент фазы в реальном потоке. |
| [`python/gait_phase_trigger.py`](python/gait_phase_trigger.py) | Обёртка фазового триггера для боевого моста. |
| [`python/stim_params.py`](python/stim_params.py) | Схема параметров стимуляции: пять интервалов, производные величины, пределы. |
| [`python/gait_score.py`](python/gait_score.py) | Счёт походки по кинематике — цель медленной петли. |
| [`python/stim_optimizer.py`](python/stim_optimizer.py) | Байесовская оптимизация с жёсткими ограничениями. |
| [`python/session_budget.py`](python/session_budget.py) | Сколько проб влезает в сессию и каким брать блок — по записям. |
...
69 тестов, около 40 секунд. Данные синтетические, GPU и записи не нужны.
Прогоняются автоматически на каждый push и PR.

Почти каждый тест закрывает ошибку, которая уже случалась. Все проверены
мутациями: если вернуть баг на место, соответствующий тест падает. Это не
формальность — мутации несколько раз показывали, что тест был декоративным и
прошёл бы при полностью выключенной проверке.

Тесты C++ плагина (gtest) запускаются отдельно, порядок в
[`docs/BUILD_PLUGIN.md`](docs/BUILD_PLUGIN.md).
...
## Тот же путь по шагам, с точными именами
...
**2026-08-19.** Фазозависимая стимуляция: процент шага в реальном времени,
интеграция в боевой мост, сторож пакетов, медленная петля (схема параметров,
счёт походки, байесовский подбор). Отчёт —
[`docs/REPORT_2026-08-19.md`](docs/REPORT_2026-08-19.md).

**2026-06-22.** Перенос наработок одно-камерного режима в dual (скользящий ROI,
оверлей, восстановление координат на пару кадров) и запись сырого видео +
keypoints с двух камер; правила достоверности кинематики в C++ плагине (полоса
угла, скорость угла, смещение точки относительно сегмента hip-ankle — гасят
ложный угловой TTL до стимуляции); запись видео переведена на lossless `.avi`
(FFV1).

**2026-06-18.** Авто-выбор видимой ноги (L/R) при развороте крысы, динамический
ROI (cudagraphs сохранён, инференс ~3.5 мс), переработанный оверлей (`DLC Hz` /
`infer ms`), параллельная запись сырого видео и keypoints (`.dlckp`/`.csv`,
фоновый поток — без влияния на инференс).
