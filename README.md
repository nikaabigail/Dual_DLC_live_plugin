...
**Версия: 2026-06-18.** Обновление одно-камерного режима: авто-выбор видимой ноги (L/R) при развороте крысы, динамический ROI по ногам (скользящее окно фикс-размера — cudagraphs сохранён, инференс ~3.5 мс), переработанный оверлей (`DLC Hz` / `infer ms`), параллельная запись сырого видео и keypoints (`.dlckp`/`.csv`, в фоновом потоке — без влияния на инференс).
