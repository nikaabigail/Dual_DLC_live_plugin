...
| | значение |
|---|---|
| попадание в цель, медиана | **−0.6% (−0.6 мс)** |
| разброс p90 | **11.9% (12.4 мс)** |
| покрытие циклов | 100% |
| импульсов на стоянке | 0 |
| частота контура | 52 Гц |

> **Поправка от 15.08.** Раньше здесь стояло p90 = 10.6% (11.1 мс). Это число
> получено с ошибкой в стенде: `realtime_phase_percent.py` добавлял смещение
> кропа к координатам, которые DLCLive уже вернул в полный кадр. На процент это
> не влияло (`relx = toe − iliac` смещение сокращает), но подмешивало движение
> окна ROI в скорость событийного детектора и случайно улучшало результат.
> Боевой мост смещения не добавляет, так что завышенное число к нему не
> относилось. После исправления и перехода на ROI 448 честная цифра 11.9%.

В метрике, принятой в поле фазозависимой стимуляции (см. [обзор алгоритмов
2024](https://pmc.ncbi.nlm.nih.gov/articles/PMC12551589/)): смещение
**−0.017 рад (−1.0°)**, accuracy (доля в пределах π/4 от цели) **96.6%**,
precision (длина результирующего вектора) **0.917**, циркулярное СКО 23.8°.
Для сравнения, лучший из сравниваемых там алгоритмов (ecHT) даёт смещение
<0.2 рад. Сигналы разные, но метрика сопоставима.

Замечание о выборе метода: Hilbert и ecHT рассчитаны на узкополосные
синусоидальные колебания, негармонические сигналы в том обзоре не
рассматривались. Локомоция это релаксационный осциллятор с duty factor 0.73,
аналитическая фаза для него плохо определена, и проекция на эталонную траекторию
это подходящее семейство методов.
...
Сверка с офлайном: полный кадр офлайн даёт p90 11.7%, реальный поток с ROI 448
даёт 11.9%. **Каузальность и живой инференс не стоят ничего по точности.**

А вот ширина ROI стоит: при ROI 256 разброс 15.0%, при 448 - 11.9%. Весь разрыв
между офлайном и потоком, который раньше приписывался каузальности, оказался
ценой узкого окна. Модель обучалась на кропах 448x448, и вход 256 px это домен,
которого она не видела. Подробности замера в `REMOTE_RESEARCH_2026-08-15.md`.
...
Раньше ~130% в перенос попасть нельзя: это первые ~29 мс переноса, а столько уходит
на кадр, инференс и детектор. Строки 105% и 110% сваливаются в одно место у пола.
Лучшая зона это 130…160%, разброс 12…17 мс.

**Как это соотносится с каноном.** Стандартное окно стимуляции сгибания это от
отрыва до середины переноса [^wenger], то есть 100…150% нашей шкалы. Задержка
контура закрывает первые ~30 п.п., так что доступны примерно 130…150%, около 40%
окна.

**Принятое решение: работаем на 145%.** Точка внутри доступной зоны, попадание
измерено (смещение −0.6%, разброс 12.4 мс). Ограничение зафиксировано как
известное; если понадобится обойти окно целиком (например для
фазово-частотной кривой), потребуется ускорение инференса - он занимает
18.6 мс из 19 мс контура.

[^wenger]: Wenger N., Moraud E.M., Gandar J. et al. Spatiotemporal neuromodulation therapies
engaging muscle synergies improve motor control after spinal cord injury.
*Nature Medicine* 22 (2016). doi:10.1038/nm.4025
